...
// bitbucketTokenURL is the Bitbucket OAuth token endpoint. Overridable in tests.
var bitbucketTokenURL = "https://bitbucket.org/site/oauth2/access_token"

// SetBitbucketTokenURLForTesting points ValidateBitbucketOAuth at a test
// server and returns a function that restores the real endpoint.
func SetBitbucketTokenURLForTesting(url string) func() {
	prev := bitbucketTokenURL
	bitbucketTokenURL = url
	return func() { bitbucketTokenURL = prev }
}
...
func ValidateBitbucketOAuth(clientID, clientSecret string) bool {
	...
	req, _ := http.NewRequest("POST", bitbucketTokenURL, body)
...
// googleTokenURL is the Google OAuth token endpoint. Overridable in tests.
var googleTokenURL = "https://www.googleapis.com/oauth2/v3/token"

// SetGoogleTokenURLForTesting points ValidateGoogleOAuth at a test server and
// returns a function that restores the real endpoint.
func SetGoogleTokenURLForTesting(url string) func() {
	prev := googleTokenURL
	googleTokenURL = url
	return func() { googleTokenURL = prev }
}
...
func ValidateGoogleOAuth(clientID, clientSecret string) bool {
	...
	req, err := http.NewRequest("POST", googleTokenURL+"?client_id="+clientID+"&client_secret="+clientSecret+"&grant_type=authorization_code&code=FAKECODE&redirect_uri=https://fakeredirect.com", nil)
	if err != nil {
		return false
	}
