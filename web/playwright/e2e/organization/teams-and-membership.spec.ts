...
test.describe('Teams and Membership', {tag: ['@organization', '@team']}, () => {
  ...
  test('non-admin user cannot edit teams', async ({readonlyPage, api}) => {
    ...
    // Add readonly user as team member (triggers invitation email)
    await api.teamMember(org.name, team.name, TEST_USERS.readonly.username);

    // Wait for and confirm the team invitation email
...
test.describe(
  ...
  () => {
    test('shows sync icon for synchronized teams', async ({
      ...
      // This test requires:
      // 1. TEAM_SYNCING feature to be enabled
      // 2. Auth type to be OIDC or LDAP (not Database)
      // 3. A synced team to exist

      // Note: Creating a synced team requires backend OIDC/LDAP configuration
      // This test placeholder assumes such configuration exists
      // The test will auto-skip if feature or auth type requirements are not met

      const org = await api.organization('teamsync');
      await api.team(org.name, 'syncteam');
      ...
      // 1. Create a team that is synced with an OIDC/LDAP group
      // 2. The API would need to support creating synced teams
