...
9. **Verify web changes with the branch's own pre-commit, not master's.** CI gates on `pre-commit`, which fails when any hook *modifies* a file — an `eslint --fix` that reports zero errors still fails it. Running `eslint` or `tsc` directly does not reproduce that, so neither counts as verification of a web change. Before reporting a web change verified, run the pre-commit job the way that branch's own CI runs it, reading the install step out of that branch's workflow rather than assuming master's: on `master` the job is in `.github/workflows/ci-lint.yaml` and installs with pnpm; on `redhat-3.16` there is no `ci-lint.yaml` at all and the job is in `.github/workflows/ci-python.yaml` with Node 18 and `cd ./web && npm ci`. Then run `pre-commit run --from-ref <base> --to-ref HEAD --verbose`. A pnpm error on a release branch means the wrong tool was used, not that the tooling is broken.
